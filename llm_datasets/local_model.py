    ...
    def _load_model(self) -> None:
        ...
        if quantize_4bit:
            try:
                ...
                logger.info(f"[{self.name}] Using 4-bit quantization (bitsandbytes)")
                model_kwargs = dict(
                ...
                logger.warning(
                    f"[{self.name}] bitsandbytes not found — falling back to float32 CPU. "
            ...
            logger.info(f"[{self.name}] Using float32 on {self.device}")
            model_kwargs = dict(
                dtype=torch.float32,
                device_map=self.device,
                low_cpu_mem_usage=self.low_cpu_mem_usage,
